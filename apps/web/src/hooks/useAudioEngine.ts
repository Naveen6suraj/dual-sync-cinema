...
export function useAudioEngine(
    ...
    const initMainAudioContext = () => {
        ...
        const mainCtx = new AudioContextClass({ 
            ...
            sampleRate: 96000 // Hi-Res Audio config
        });
    ...
    const connectListenerPipeline = (listener: Listener) => {
        ...
        try {
            ...
            const audioElWithSink = audioElement as unknown as { setSinkId?: (id: string) => Promise<void> };
            if (listener.deviceId && listener.deviceId !== 'default' && typeof audioElWithSink.setSinkId === 'function') {
                audioElWithSink.setSinkId(listener.deviceId).catch((err: unknown) => {
            ...
            prev.map(l => {
                if (l.id === id) {
                    ...
                    if (updated.audioElement) {
                        const audioElWithSink = updated.audioElement as unknown as { setSinkId?: (id: string) => Promise<void> };
                        if (typeof audioElWithSink.setSinkId === 'function') {
                            audioElWithSink.setSinkId(deviceId)
    ...
    const saveDeviceProfile = async (listener: Listener, userId: string) => {
        try {
            // Try saving to DB if JWT is available, else fallback to localStorage
            const payload = { deviceName: listener.name, delayOffsetMs: listener.delayMs };
            localStorage.setItem(`syncProfile_${userId}_${listener.name}`, JSON.stringify(payload));
            
            // In a real implementation this would fetch to /api/profiles
            console.log(`Saved profile for ${listener.name} with delay ${listener.delayMs}ms`);
        } catch (e) {
            console.error('Failed to save device profile', e);
        }
    };

    const loadDeviceProfile = (listenerName: string, userId: string): number | null => {
        try {
            const data = localStorage.getItem(`syncProfile_${userId}_${listenerName}`);
            if (data) {
                const parsed = JSON.parse(data);
                return parsed.delayOffsetMs;
            }
        } catch (e) {
            console.error('Failed to load device profile', e);
        }
        return null;
    };
